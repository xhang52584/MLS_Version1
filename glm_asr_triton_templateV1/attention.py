...
def attention_scores_kernel(
    ...
    *** TODO: Implement this kernel ***
    ...
    curr_q_base = q_ptr + pid_bh * stride_q0 + pid_q * stride_q1
    curr_k_base = k_ptr + pid_bh * stride_k0
    curr_s_base = scores_ptr + pid_bh * stride_s0 + pid_q * stride_s1

    # Step 1: Load query vector for this position
    q = tl.load(
        curr_q_base + offs_d * stride_q2,
    ...
    # Step 3: Compute dot-product scores and scale
    scores = tl.sum(k * q_scaled[None, :], axis=1)

    # Step 4: Store scores
    tl.store(
        curr_s_base + offs_k * stride_s2,
        scores,
        mask=offs_k < seq_k,
    )
...
def softmax_inplace_kernel(scores_ptr, stride_s, seq_k, BLOCK_SIZE: tl.constexpr):
    ...
    #Get the base address
    curr_s_base = scores_ptr + row * stride_s

    # Step 1: Load scores row with masking
    s = tl.load(curr_s_base + offs, mask=mask, other=-float("inf"))
    ...
    # Step 4: Store back
    tl.store(curr_s_base + offs, out, mask=mask)
...
def attention_output_kernel(
    ...
    #Get the base address
    curr_w_base = attn_ptr + pid_bh * stride_w0 + pid_q * stride_w1
    curr_v_base = v_ptr + pid_bh * stride_v0
    curr_o_base = output_ptr + pid_bh * stride_o0 + pid_q * stride_o1

    # Step 1: Load attention weights for this query
    w = tl.load(
        curr_w_base + offs_k * stride_w2,
        mask=offs_k < seq_k,
        other=0.0,
    )
